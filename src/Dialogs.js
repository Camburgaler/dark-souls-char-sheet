...
function Dialogs(props) {
    ...
    const proficiencySelectionDialogError = Object.values(props.proficiencySelections).filter((v) => v).length !== PROFICIENCY_SELECTION[props.character.selectedClass];
    const deprivedSavingThrowDialogError = Object.values(props.deprivedSavingThrows).filter((v) => v).length !== 2;
    ...
    const handleDeprivedSavingThrowSubmit = () => {
        if (deprivedSavingThrowDialogError) {
            ...
            props.setCharacter({
                ...
                proficiencies: {
                    ...props.character.proficiencies,
                    strSavingThrow: props.deprivedSavingThrows.deprivedStrSavingThrow,
                    dexSavingThrow: props.deprivedSavingThrows.deprivedDexSavingThrow,
                    conSavingThrow: props.deprivedSavingThrows.deprivedConSavingThrow,
                    intSavingThrow: props.deprivedSavingThrows.deprivedIntSavingThrow,
                    wisSavingThrow: props.deprivedSavingThrows.deprivedWisSavingThrow,
                    chaSavingThrow: props.deprivedSavingThrows.deprivedChaSavingThrow
                }
            });
            props.setDeprivedSavingThrowsDialogOpen(false);
        }
    ...
    const handleCombatPositionSubmitButtonClick = () => {
        if (combatPositionText < props.character.level || combatPositionText > props.character.level * props.character.positionDice.diceType) {
            //do nothing
        } else {
            props.setCharacter({
                ...props.character,
                position: {
                    ...
                    combatPool: props.character.position.base + combatPositionText,
                    current: props.character.position.current + combatPositionText
                }
            });
            props.setCombatPositionDialogOpen(false);
        }
    ...
    const handlePositionRecoverySubmitButtonClick = () => {
        if (positionRecoveryText < 1 || positionRecoveryText > props.character.positionDice.diceType) {
            //do nothing
        } else {
            if (props.character.position.current + positionRecoveryText > props.character.position.combatPool) {
                props.character.position.current = props.character.position.combatPool;
            } else {
                props.character.position.current = props.character.position.current + positionRecoveryText;
            }
    ...
    const handleCombatPositionAutoRollButtonClick = () => {
        const max = props.character.level * props.character.positionDice.diceType;
    ...
    const handlePositionRecoveryAutoRollButtonClick = () => {
        ...
        if (props.character.position.current + positionRecovery > props.character.position.combatPool) {
            props.character.position.current = props.character.position.combatPool;
        } else {
            props.character.position.current = props.character.position.current + positionRecovery;
        }
    ...
    return (
        <>
            <Dialog onClose={handleProficiencySelectionDialogClose} open={props.proficiencySelectionDialogOpen}>
                <DialogTitle>Select {PROFICIENCY_SELECTION[props.character.selectedClass]} Proficiencies</DialogTitle>
                <FormControl
                    required
                ...
                >
                    <FormGroup>
                        {(CLASS_PROFICIENCIES[props.character.selectedClass]).map((proficiency) => {
                            return (<FormControlLabel
                                key={proficiency}
                                control={
                                    <Checkbox checked={props.proficiencySelections.proficiency} onClick={handleProficiencySelectionClick} name={proficiency} />
                                }
            ...
            <Dialog onClose={handleDeprivedDialogClose} open={props.deprivedSavingThrowsDialogOpen}>
                    ...
                    <FormGroup>
                        {Object.entries(ABILITY_NAMES).map((ability) => {
                            return (<FormControlLabel
                                key={ability[0]}
                                control={
                                    <Checkbox checked={props.deprivedSavingThrows[ability[0]]} onClick={handleDeprivedSavingThrowClick} name={"deprived" + ability[1].substr(0, 3) + "SavingThrow"} />
                                }
                                label={ability[1]}
                            />)
                        })}
            ...
            <Dialog open={props.combatPositionDialogOpen} onClose={handleCombatPositionDialogClose}>
                <DialogTitle>Roll for Combat Position Pool!</DialogTitle>
                <Grid container direction="column">
                    <Grid item xs={12}>
                        <Typography>Roll {props.character.level}d{props.character.positionDice.diceType} position dice and enter the result below.</Typography>
                    </Grid>
            ...
            <Dialog open={props.positionRecoveryDialogOpen} onClose={handlePositionRecoveryDialogClose}>
                <DialogTitle>Roll for Position Recovery</DialogTitle>
                <Grid container direction="column">
                    <Grid item xs={12}>
                        <Typography>Roll 1d{props.character.positionDice.diceType} position dice and enter the result below.</Typography>
                    </Grid>
                    <Grid container direction="row">
                        <Grid item xs={9}>
                            <TextField fullWidth name="positionRecoveryInput" type="number" onChange={handlePositionRecoveryTextChange}></TextField>
                        </Grid>
                        <Grid item xs={3}>
                            <Button name="positionRecoverySubmitButton" onClick={handlePositionRecoverySubmitButtonClick} fullWidth>Submit</Button>
                        </Grid>
                    </Grid>
                    <Grid item xs={12}>
                        <Button name="positionRecoveryAutoRollButton" onClick={handlePositionRecoveryAutoRollButtonClick} fullWidth>Roll for me!</Button>
                    </Grid>
                </Grid>
            </Dialog>
        </>
